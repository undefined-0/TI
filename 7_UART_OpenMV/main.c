// 注：使用本程序时，OpenMV发送的命令结尾要追加\n，MSP430才能正常接收到
// 对于本开发板（MSP430G2553 Launch Pad），P1.1是RX，P1.2是TX
// 接线方法：MSP430 P1.1 <---> OpenMV P4
//          MSP430 P1.2 <---> OpenMV P5
...
#include "oled.h"
#include "bmp.h"
uint8_t receive_cmd[20] = {0};//长度为20的命令缓冲区，用于保存串口接收到的命令
...
void cmd_judge_execute(uint8_t *receive_cmd)
{
    ...
    if(receive_cmd[0] == preset_cmd[0][0] && receive_cmd[6] == preset_cmd[0][6])// 接收到LED1 ON
    {
        ...
        OLED_Clear();
        OLED_ShowString(0,0,"LED1 has been",14);
        OLED_ShowString(0,2,"illuminated",12);
        P1OUT |= BIT0;
    }
    else if(receive_cmd[0] == preset_cmd[1][0] && receive_cmd[6] == preset_cmd[1][6])// 接收到LED1 OFF
    {
        ...
        OLED_Clear();
        OLED_ShowString(0,0,"LED1 has been",14);
        OLED_ShowString(0,2,"turned off",11);
        P1OUT &= ~BIT0;
    }
    else
    {
        ...
        OLED_Clear();
        OLED_ShowString(0,0,"Not a preset",13);
        OLED_ShowString(0,2,"command",8);
